...
영구노트 본문도 정본이 있다. 절 순서는 날짜 → 태그 → 메모 → 원문 → 생각 → [추천] → 연결이다. `추천 (주제)`는 선택 절이고, `연결 (이유)`는 필수이며 언제나 마지막 절이다.
...
다른 서식으로 적힌 옛 노트는 `skills/_shared/scripts/normalize.py`가 이 서식으로 옮긴다. 헤더만 바뀌고 텍스트 줄은 하나도 버리지 않으며, 옮긴 뒤 `assert_text_preserved()`가 그것을 검사한다. 표에 없는 헤더는 `### 생각 (질문)` 아래 `**원헤더**` 소제목으로 남는다. 변환은 `추천 (주제)`를 만들지 않고, 빠진 `연결 (이유)`만 빈 절로 채운다.

이미 스타일 A인 노트는 **절 순서를 재배열하지 않는다.** `### 날짜 :`·`### 태그 :` 줄을 frontmatter 값으로 보정하고 빠진 `연결 (이유)` 빈 절을 끝에 붙이는 것이 전부다. `추천`이 `연결`보다 앞이든 뒤든 그대로 둔다.
