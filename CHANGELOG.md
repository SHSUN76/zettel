...
- 영구노트 본문 정본이 스타일 A(`### 날짜` · `### 태그` · `>[!메모]` · `### 원문 (출처)` · `### 생각 (질문)` · [`### 추천 (주제)`] · `### 연결 (이유)`)다. `추천 (주제)` 는 선택 절이고 `연결 (이유)` 는 필수이며 마지막 절이다. `promote`·`literature` 가 이 서식으로 만들고, `normalize` 는 이미 스타일 A 인 노트의 절 순서를 재배열하지 않는다.
